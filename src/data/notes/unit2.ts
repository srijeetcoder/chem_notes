export const unit2Notes = [
  ...
  },
  {
    ...
    explanation: "Chemical bonds can be modeled as a two-mass system connected by a spring. 1. Hooke's Law: The potential energy is E = 0.5 * k * x², where x = r₂ - r₁ is the bond elongation, and k is the force constant. The vibrational frequency is f = (1 / 2π) * √(k/μ), where μ = m₁m₂/(m₁+m₂) is the reduced mass. 2. Harmonic Oscillator: Schrödinger equation yields quantized energy levels: E = (v + 0.5) * hf, where v is the vibrational quantum number (v = 0, 1, 2...). 3. Anharmonic Oscillator (Real Case): The energy is E = (v + 0.5)hf - (v + 0.5)² hfχ_e, where χ_e is the anharmonic constant. 4. Dissociation Energy: Differentiating energy E with respect to v and setting it to 0 gives dE/dv = hf - 2(v_max + 0.5)hfχ_e = 0, yielding v_max = 1/(2χ_e) - 1/2. Substituting v_max back gives the maximum bond energy, which is the Bond Dissociation Energy: E_max = D_e = hf / (4χ_e). The Spectroscopic Dissociation Energy (energy to break bond from v=0) is D_0 = D_e - E_0, where the zero-point energy is E_0 = 0.5 * hf - 0.25 * hfχ_e.\n\nIR Activity & shifts: Homonuclear molecules (N₂, O₂, Cl₂) are IR-inactive. Carbonyl (C=O) frequencies are strongly affected by:\n- Ring Strain (Bent's Rule): In cyclic ketones, decreasing bond angles (Cyclohexanone: 1715 cm⁻¹, Cyclopentanone: 1745 cm⁻¹, Cyclobutanone: 1780 cm⁻¹, Cyclopropenone: 1850 cm⁻¹) concentrates s-character in the C=O bond, making the double bond act more like a triple bond, raising frequency.\n- Substituent Effects: Aldehyde (1750 cm⁻¹) has no alkyl effects. Ketone (1715 cm⁻¹) has +I donor alkyl groups that weaken the C=O bond. Ester (1735 cm⁻¹) has an alkoxy group where the electronegative oxygen's -I effect dominates over +R resonance, strengthening the bond. Amide (1690 cm⁻¹) has a strong +R nitrogen donor resonance, imparting single-bond character and lowering frequency.\n\nCO₂ has 4 vibrational modes:\n1. Symmetric Stretching: <- O=C=O -> (Dipole change = 0, IR-inactive)\n2. Asymmetric Stretching: -> O=C=O -> (Dipole change ≠ 0, IR-active)\n3. Perpendicular Bending: ↓ O = ↑ C = ↓ O (Dipole change ≠ 0, IR-active)\n4. Anti-perpendicular Bending: ⊗ O = ⊙ C = ⊗ O (Dipole change ≠ 0, IR-active)\nH₂O has 3 modes: Symmetric stretching, Asymmetric stretching, and Symmetric bending (all are IR-active).",
    formulas: [
